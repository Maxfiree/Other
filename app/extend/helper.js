...
const helper = {
  ...
  createJwtPayload(user, jwtSignOption) {
    ...
    const data = {};
    data.user = R.pick([ 'id', 'username', 'name', 'phone', 'email', 'headImgURL',
      'gender', 'status', 'accessToken', 'refreshToken', 'roleName', 'groupName' ], user);
    ...
    this.ctx.cookies.set('jwt', jwt.sign(data, this.app.config.passportJWT.jwtKey, options), { httpOnly: false });
  },
  /**
